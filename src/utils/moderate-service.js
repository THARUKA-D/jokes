...
const signIn = (payload, isLoading, navigateTo) => {
  isLoading(true);
  axios.post('http://localhost:3002/signIn', payload)
    .then(response => {
...
const fetchUnmoderatedJoke = (isLoading, setJokeData) => {
  isLoading(true);
  axios.get(
    'http://localhost:3002/getNewJoke',
    {
      headers: {
...
const addModeratedJoke = (
  ...
  isLoading(true);
  axios.post(
    'http://localhost:3002/addNewJoke',
    payload,
    {
      headers: {
        Authorization: getAuthToken(),
...
const deleteJoke = (
  ...
  setIsLoading(true);
  axios.post(
    'http://localhost:3002/deleteJoke',
    payload,
    {
      headers: {
        Authorization: getAuthToken(),
