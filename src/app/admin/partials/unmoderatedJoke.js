...
export default function UnmoderatedJoke() {
  ...
  const renderJoke = () => {
    if (!jokeData.length) {
      return <Typography>No new jokes available.</Typography>;
    }
    const joke = jokeData[0];

    return (
      <SubmitJoke isAdmin={true} jokeData={joke} onCompleteAction={onGetJoke} />
    );
  };
  ...
  return (
    ...
    >
      <Stack spacing={2}>
        {renderJoke()}
        <Button variant="contained" onClick={onGetJoke}>
          Next Joke
        </Button>
      </Stack>
    </Card>
