...
export default function SubmitJoke({
  ...
  const onCustomTypeChange = event => {
    ...
    if (jokeTypes) {
      const existingJokeType = jokeTypes.find(jokeType => jokeType.JokeType.toLowerCase() === value.toLowerCase());
      if (existingJokeType) {
  ...
  useEffect(() => {
    if (jokeTypeId === IS_CUSTOM_JOKE_TYPE
  ...
  const renderButtons = () => {
    if (isAdmin) {
      ...
      >
        <Button
          variant="contained"
          onClick={() => { setTextFieldsDisabled(false) }}
        >
          Edit
        </Button>
  ...
  const renderJokeDelivery = () => {
    if (
      (isAdmin && jokeData.jokeDelivery.length)
      || !isAdmin) {
      return true;
    }
  ...
  const renderJokeTypes = () => {
    ...
    return <FormControl fullWidth>
      ...
      >
        {jokeTypes && jokeTypes.map(jokeType => <MenuItem value={jokeType.Id}>
          {jokeType.JokeType}
        </MenuItem>)
        }
